...
// source: https://stackoverflow.com/questions/17399119/how-do-i-use-atomicmax-on-floating-point-values-in-musa
...
// taken from https://stackoverflow.com/a/4609795, but not sure if there's a proper way to do this in MUSA
...
__launch_bounds__(1024, BNB_MAX_THREADS_PER_SM / 1024) __global__
    ...
    // For sm50/sm52 and MUSA < 12.2 we need to do the reduction in fp32.
    // Otherwise `T` is `fp16`. This can be removed when Maxwell is dropped.
#if (__MUSACC_VER_MAJOR__ >= 12 && __MUSACC_VER_MINOR >= 2) || BNB_FP16_AVAILABLE
    using TReduction = T;
#else
...
#if __MUSA_ARCH__ >= 750
    using namespace nvmusa;
...
template __global__ void kgemm_4bit_inference_naive<__mt_bfloat16, 128, 16>(
    int M, int N, int K, __mt_bfloat16* __restrict__ const A, unsigned char* B, float* absmax, const float* datatype,
    __mt_bfloat16* out, int lda, int ldb, int ldc, int blocksize
);
...
MAKE_PreconditionOptimizer32bit2State(ADAM, __mt_bfloat16)
MAKE_PreconditionOptimizer32bit2State(ADEMAMIX, float)
MAKE_PreconditionOptimizer32bit2State(ADEMAMIX, half)
MAKE_PreconditionOptimizer32bit2State(ADEMAMIX, __mt_bfloat16)
...
template __global__ void kOptimizer32bit2State<__mt_bfloat16, ADAM>(
    __mt_bfloat16* g, __mt_bfloat16* p, float* state1, float* state2, float* unorm, const float max_unorm,
...
template __global__ void kOptimizer32bit2State<__mt_bfloat16, ADEMAMIX>(
    __mt_bfloat16* g, __mt_bfloat16* p, float* state1, float* state2, float* unorm, const float max_unorm,
    const float param_norm, const float beta1, const float beta2, const float beta3, const float alpha, const float eps,
    const float weight_decay, const int step, const float lr, const float gnorm_scale, const bool skip_zeros,
    const int n
...
MAKE_kQuantizeBlockwise(__mt_bfloat16, 4096, 4, 0, General8bit)
MAKE_kQuantizeBlockwise(__mt_bfloat16, 4096, 4, 1, General8bit)
MAKE_kQuantizeBlockwise(__mt_bfloat16, 2048, 4, 0, General8bit)
MAKE_kQuantizeBlockwise(__mt_bfloat16, 1024, 4, 0, General8bit)
MAKE_kQuantizeBlockwise(__mt_bfloat16, 512, 2, 0, General8bit)
MAKE_kQuantizeBlockwise(__mt_bfloat16, 256, 2, 0, General8bit)
MAKE_kQuantizeBlockwise(__mt_bfloat16, 128, 2, 0, General8bit)
MAKE_kQuantizeBlockwise(__mt_bfloat16, 64, 2, 0, General8bit)
MAKE_kQuantizeBlockwise(__mt_bfloat16, 4096, 4, 0, FP4)
MAKE_kQuantizeBlockwise(__mt_bfloat16, 2048, 4, 0, FP4)
MAKE_kQuantizeBlockwise(__mt_bfloat16, 1024, 4, 0, FP4)
MAKE_kQuantizeBlockwise(__mt_bfloat16, 512, 2, 0, FP4)
MAKE_kQuantizeBlockwise(__mt_bfloat16, 256, 2, 0, FP4)
MAKE_kQuantizeBlockwise(__mt_bfloat16, 128, 2, 0, FP4)
MAKE_kQuantizeBlockwise(__mt_bfloat16, 64, 2, 0, FP4)
MAKE_kQuantizeBlockwise(__mt_bfloat16, 4096, 4, 0, NF4)
MAKE_kQuantizeBlockwise(__mt_bfloat16, 2048, 4, 0, NF4)
MAKE_kQuantizeBlockwise(__mt_bfloat16, 1024, 4, 0, NF4)
MAKE_kQuantizeBlockwise(__mt_bfloat16, 512, 2, 0, NF4)
MAKE_kQuantizeBlockwise(__mt_bfloat16, 256, 2, 0, NF4)
MAKE_kQuantizeBlockwise(__mt_bfloat16, 128, 2, 0, NF4)
MAKE_kQuantizeBlockwise(__mt_bfloat16, 64, 2, 0, NF4)
...
template __global__ void kDequantizeBlockwise<__mt_bfloat16, 512, 64, 8, FP4>(
    float* code, unsigned char* A, float* absmax, __mt_bfloat16* out, const int blocksize, const int n
);
template __global__ void kDequantizeBlockwise<__mt_bfloat16, 512, 64, 8, General8bit>(
    float* code, unsigned char* A, float* absmax, __mt_bfloat16* out, const int blocksize, const int n
);
template __global__ void kDequantizeBlockwise<__mt_bfloat16, 512, 64, 8, NF4>(
    float* code, unsigned char* A, float* absmax, __mt_bfloat16* out, const int blocksize, const int n
);
...
MAKE_OptimizerStatic8bit2StateBlockwise(ADAM, __mt_bfloat16, 256, 1)
MAKE_OptimizerStatic8bit2StateBlockwise(ADEMAMIX, float, 256, 1)
MAKE_OptimizerStatic8bit2StateBlockwise(ADEMAMIX, half, 256, 1)
MAKE_OptimizerStatic8bit2StateBlockwise(ADEMAMIX, __mt_bfloat16, 256, 1)
