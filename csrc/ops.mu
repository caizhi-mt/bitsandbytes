...
void quantize(float* code, float* A, unsigned char* out, int n) {
    ...
    MUSA_CHECK_RETURN(musaPeekAtLastError());
}

void dequantize(float* code, unsigned char* A, float* out, int n, musaStream_t stream) {
    int num_blocks = n / 1024;
    num_blocks = n % 1024 == 0 ? num_blocks : num_blocks + 1;
...
void dequantizeBlockwise(
    float* code, unsigned char* A, float* absmax, T* out, int blocksize, const int n, musaStream_t stream
) {
    // printf("stream==%d\n",stream);
    int num_blocks = n / blocksize;
    num_blocks = n % blocksize == 0 ? num_blocks : num_blocks + 1;
...
void optimizer32bit(
    ...
    case ADEMAMIX:
        if (max_unorm > 0.0f) {
            MUSA_CHECK_RETURN(musaMemset(unorm, 0, 1 * sizeof(float)));
...
void optimizerStatic8bit(
    ...
    switch (OPTIMIZER) {
    case ADAM:
        MUSA_CHECK_RETURN(musaMemset(new_max1, 0, 1 * sizeof(float)));
        MUSA_CHECK_RETURN(musaMemset(new_max2, 0, 1 * sizeof(float)));
...
template <typename T> void percentileClipping(T* g, float* gnorm_vec, int step, const int n) {
    ...
    MUSA_CHECK_RETURN(musaMemset(&gnorm_vec[step % 100], 0, 1 * sizeof(float)));
...
void gemmex(
    ...
    status = mublasGemmEx(
        context->m_handle, transposeA ? MUBLAS_OP_T : MUBLAS_OP_N, transposeB ? MUBLAS_OP_T : MUBLAS_OP_N, m, n, k,
        alpha, A, MUSA_R_8I, lda, B, MUSA_R_8I, ldb, beta, C, MUSA_R_32I, ldc, MUSA_R_32I, MUBLAS_GEMM_DEFAULT_TENSOR_OP
    );

    if (status != MUBLAS_STATUS_SUCCESS) {
        std::cout << "MUBLAS ERROR: Status " << status << std::endl;
    }
}
...
void strided_gemmex(
    ...
    status = mublasGemmStridedBatchedEx(
        context->m_handle, transposeA ? MUBLAS_OP_T : MUBLAS_OP_N, transposeB ? MUBLAS_OP_T : MUBLAS_OP_N, m, n, k,
        alpha, A, MUSA_R_8I, lda, (long long int)strideA, B, MUSA_R_8I, ldb, (long long int)strideB, beta, C,
        MUSA_R_32I, ldc, (long long int)strideC, batchCount, MUSA_R_32I, MUBLAS_GEMM_DEFAULT
    );
...
template <int ORDER> mublasLtOrder_t get_order() {
    switch (ORDER) {
    case ROW:
        return MUBLASLT_ORDER_ROW;
        break;
    case COL:
        return MUBLASLT_ORDER_COL;
        break;
    case COL32:
        return MUBLASLT_ORDER_COL32;
        break;
    case COL_TURING:
        return MUBLASLT_ORDER_COL4_4R2_8C;
        break;
    case COL_AMPERE:
        return MUBLASLT_ORDER_COL32_2R_4R4;
        break;
    default:
        break;
    }
...
template mublasLtOrder_t get_order<ROW>();
template mublasLtOrder_t get_order<COL>();
template mublasLtOrder_t get_order<COL32>();
template mublasLtOrder_t get_order<COL_TURING>();
template mublasLtOrder_t get_order<COL_AMPERE>();
...
int igemmlt(
    mublasLtHandle_t ltHandle, int m, int n, int k, const int8_t* A, const int8_t* B, void* C, float* row_scale,
    int lda, int ldb, int ldc, musaStream_t stream
) {
    ...
    mublasLtMatrixLayout_t aDesc, bDesc, cDesc;
    mublasOperation_t opT = MUBLAS_OP_T;

    musaDataType_t outType = DTYPE_OUT == 32 ? MUSA_R_32I : MUSA_R_8I;
    musaDataType_t scaleType = DTYPE_OUT == 32 ? MUSA_R_32I : MUSA_R_32F;

    mublasLtPointerMode_t pointerMode = MUBLASLT_POINTER_MODE_ALPHA_DEVICE_VECTOR_BETA_ZERO;

    has_error |= checkCublasStatus(mublasLtMatrixLayoutCreate(&aDesc, MUSA_R_8I, m, k, lda));
    has_error |= checkCublasStatus(mublasLtMatrixLayoutCreate(&bDesc, MUSA_R_8I, m, n, ldb));
    has_error |= checkCublasStatus(mublasLtMatrixLayoutCreate(&cDesc, outType, k, n, ldc));

    // Default layout order is col major

    has_error |= checkCublasStatus(mublasLtMatmulDescCreate(&matmulDesc, MUBLAS_COMPUTE_32I, scaleType));
    has_error |=
        checkCublasStatus(mublasLtMatmulDescSetAttribute(matmulDesc, MUBLASLT_MATMUL_DESC_TRANSA, &opT, sizeof(opT)));

    if (DTYPE_OUT == 32) {
        int alpha = 1, beta = 0;
        has_error |= checkCublasStatus(mublasLtMatmul(
        ...
        if (!SCALE_ROWS) {
            ...
            mublasLtPointerMode_t alphaVec = MUBLASLT_POINTER_MODE_ALPHA_DEVICE_VECTOR_BETA_HOST;
            float beta = 0.0f;
            has_error |= checkCublasStatus(mublasLtMatmulDescSetAttribute(
                matmulDesc, MUBLASLT_MATMUL_DESC_POINTER_MODE, &pointerMode, sizeof(alphaVec)
            ));
            has_error |= checkCublasStatus(mublasLtMatmul(
    ...
    has_error |= checkCublasStatus(mublasLtMatrixLayoutDestroy(cDesc));
    has_error |= checkCublasStatus(mublasLtMatrixLayoutDestroy(bDesc));
    has_error |= checkCublasStatus(mublasLtMatrixLayoutDestroy(aDesc));
    has_error |= checkCublasStatus(mublasLtMatmulDescDestroy(matmulDesc));
...
void dequant_mm_int32_fp16(
    int* A, float* rowStats, float* colStats, half* out, half* bias, int numRows, int numCols, musaStream_t stream
) {
...
void int8VectorQuant(
    half* __restrict__ A, int8_t* out, float* rowStats, float threshold, int rows, int cols, musaStream_t stream
) {
    if (threshold == 0.0) {
...
void getRowStats(half* A, float* rowStats, float threshold, int rows, int cols, musaStream_t stream) {
    if (threshold == 0.0)
...
void spmm_coo(
    musparseHandle_t handle, int* A_rowidx, int* A_colidx, half* A_vals, int A_nnz, int A_rows, int A_cols, int B_cols,
    int ldb, half* B, int ldc, half* C, bool transposed_B
) {
    musparseSpMatDescr_t descA;
    musparseDnMatDescr_t descB, descC;
    ...
    CHECK_MUSPARSE(musparseCreateCoo(
        &descA, A_rows, A_cols, A_nnz, A_rowidx, A_colidx, A_vals, MUSPARSE_INDEX_32I, MUSPARSE_INDEX_BASE_ZERO,
        MUSA_R_16F
    ));
    // Create dense matrix C
    CHECK_MUSPARSE(musparseCreateDnMat(&descC, A_rows, B_cols, ldc, C, MUSA_R_16F, MUSPARSE_ORDER_ROW));
    ...
    CHECK_MUSPARSE(musparseCreateDnMat(&descB, A_cols, B_cols, ldb, B, MUSA_R_16F, MUSPARSE_ORDER_ROW));
    // allocate an external buffer if needed
// =============================================================================================================================
//    CHECK_MUSPARSE(musparseSpMM_bufferSize(
//        handle, MUSPARSE_OPERATION_NON_TRANSPOSE,
//        transposed_B ? MUSPARSE_OPERATION_TRANSPOSE : MUSPARSE_OPERATION_NON_TRANSPOSE, &alpha, descA, descB, &beta,
//        descC, MUSA_R_32F, MUSPARSE_SPMM_ALG_DEFAULT, &bufferSize
//    ));
// =============================================================================================================================
    MUSA_CHECK_RETURN(musaMalloc(&dBuffer, bufferSize));

    // execute SpMM
//    CHECK_MUSPARSE(musparseSpMM(
//        handle, MUSPARSE_OPERATION_NON_TRANSPOSE,
//        transposed_B ? MUSPARSE_OPERATION_TRANSPOSE : MUSPARSE_OPERATION_NON_TRANSPOSE, &alpha, descA, descB, &beta,
//        descC, MUSA_R_32F, MUSPARSE_SPMM_ALG_DEFAULT, dBuffer
//    ));

    // destroy matrix/vector descriptors
    CHECK_MUSPARSE(musparseDestroySpMat(descA));
    CHECK_MUSPARSE(musparseDestroyDnMat(descB));
    CHECK_MUSPARSE(musparseDestroyDnMat(descC));
    MUSA_CHECK_RETURN(musaFree(dBuffer));
}
...
void gemm_4bit_inference_naive(
    ...
    int blocksize, musaStream_t stream
) {
...
template void gemm_4bit_inference_naive<half, 16>(
    ...
    int ldc, int blocksize, musaStream_t stream
);
template void gemm_4bit_inference_naive<__mt_bfloat16, 16>(
    int m, int n, int k, __mt_bfloat16* A, unsigned char* B, float* absmax, float* datatype, __mt_bfloat16* out,
    int lda, int ldb, int ldc, int blocksize, musaStream_t stream
);
...
template void quantizeBlockwise<__mt_bfloat16, 1, General8bit>(
    float* code, __mt_bfloat16* A, float* absmax, unsigned char* out, float* rand, int rand_offset, int blocksize,
    const int n
);
template void quantizeBlockwise<__mt_bfloat16, 0, General8bit>(
    float* code, __mt_bfloat16* A, float* absmax, unsigned char* out, float* rand, int rand_offset, int blocksize,
    const int n
);
template void quantizeBlockwise<__mt_bfloat16, 0, FP4>(
    float* code, __mt_bfloat16* A, float* absmax, unsigned char* out, float* rand, int rand_offset, int blocksize,
    const int n
);
template void quantizeBlockwise<__mt_bfloat16, 0, NF4>(
    float* code, __mt_bfloat16* A, float* absmax, unsigned char* out, float* rand, int rand_offset, int blocksize,
    const int n
);

template void dequantizeBlockwise<float, General8bit>(
    float* code, unsigned char* A, float* absmax, float* out, int blocksize, const int n, musaStream_t stream
);
...
template void dequantizeBlockwise<half, General8bit>(
    float* code, unsigned char* A, float* absmax, half* out, int blocksize, const int n, musaStream_t stream
);
...
template void dequantizeBlockwise<__mt_bfloat16, General8bit>(
    float* code, unsigned char* A, float* absmax, __mt_bfloat16* out, int blocksize, const int n, musaStream_t stream
);
template void dequantizeBlockwise<__mt_bfloat16, FP4>(
    float* code, unsigned char* A, float* absmax, __mt_bfloat16* out, int blocksize, const int n, musaStream_t stream
);
template void dequantizeBlockwise<__mt_bfloat16, NF4>(
    float* code, unsigned char* A, float* absmax, __mt_bfloat16* out, int blocksize, const int n, musaStream_t stream
);
...
MAKE_optimizer32bit(ADAM, half) MAKE_optimizer32bit(ADAM, float) MAKE_optimizer32bit(ADAM, __mt_bfloat16) MAKE_optimizer32bit(MOMENTUM, half) MAKE_optimizer32bit(MOMENTUM, float) MAKE_optimizer32bit(
    MOMENTUM, __mt_bfloat16
) MAKE_optimizer32bit(RMSPROP, half) MAKE_optimizer32bit(RMSPROP, float) MAKE_optimizer32bit(RMSPROP, __mt_bfloat16) MAKE_optimizer32bit(LION, half) MAKE_optimizer32bit(LION, float) MAKE_optimizer32bit(LION, __mt_bfloat16) MAKE_optimizer32bit(ADAGRAD, half) MAKE_optimizer32bit(ADAGRAD, float) MAKE_optimizer32bit(ADAGRAD, __mt_bfloat16) MAKE_optimizer32bit(ADEMAMIX, half) MAKE_optimizer32bit(ADEMAMIX, __mt_bfloat16) MAKE_optimizer32bit(ADEMAMIX, float)
...
MAKE_optimizerStatic8bitBlockwise(__mt_bfloat16, ADAM);
MAKE_optimizerStatic8bitBlockwise(half, MOMENTUM);
MAKE_optimizerStatic8bitBlockwise(float, MOMENTUM);
MAKE_optimizerStatic8bitBlockwise(__mt_bfloat16, MOMENTUM);
MAKE_optimizerStatic8bitBlockwise(half, RMSPROP);
MAKE_optimizerStatic8bitBlockwise(float, RMSPROP);
MAKE_optimizerStatic8bitBlockwise(__mt_bfloat16, RMSPROP);
MAKE_optimizerStatic8bitBlockwise(half, LION);
MAKE_optimizerStatic8bitBlockwise(float, LION);
MAKE_optimizerStatic8bitBlockwise(__mt_bfloat16, LION);
MAKE_optimizerStatic8bitBlockwise(half, ADAGRAD);
MAKE_optimizerStatic8bitBlockwise(float, ADAGRAD);
MAKE_optimizerStatic8bitBlockwise(__mt_bfloat16, ADAGRAD);
MAKE_optimizerStatic8bitBlockwise(half, ADEMAMIX);
MAKE_optimizerStatic8bitBlockwise(__mt_bfloat16, ADEMAMIX);
